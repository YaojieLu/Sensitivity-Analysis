...
mtext(expression(italic(over(dA, dg[s]))*"  or  "*italic(over(partialdiff*m, partialdiff*g[s]))), side=2, line=0.2, cex=1.3)
text(0.4*0.05/8*6, 40*0.95, "c", cex=1.5)

for(i in 1:length(SA)){
  Vcmax <- SA[i]
  curve(dAdgsf, 0, 0.4, add=T, col=Cols[i])
  curve(dmdgsf, 0, 0.4, add=T, lty=2)
}

legend("topright", title=expression(italic(V[cmax])), legend=SA, col=Cols, lty=c(1), bg="white")
box()
