...
# average A
plotBy(data$averA ~ data$MAP | Vcmax, data=subset(data, k=="0.025"),
...
plotBy(data$averA ~ data$MAP | Vcmax, data=subset(data, k=="0.1"),
       type='l', legend=FALSE, legendwhere="topleft",
       xlim=c(0, 3000), ylim=c(0, 25),
       xlab=NA, ylab=NA,
       xaxt="n", yaxt="n",
...
legend("topleft", title="Vcmax", c("25", "50", "75"), col=Cols, pch=19)

# E/MAP
plotBy(data$EMAP ~ data$MAP | Vcmax, data=subset(data, k=="0.025"),
       type='l', legend=FALSE, legendwhere="topleft",
...
plotBy(data$EMAP ~ data$MAP | Vcmax, data=subset(data, k=="0.1"),
       type='l', legend=FALSE, legendwhere="topleft",
       xlim=c(0, 3000), ylim=c(0, 1),
       xlab=NA, ylab=NA,
       xaxt="n", yaxt="n",
...
dev.copy2pdf(file = "Figures/Averages/Vcmax.pdf")
