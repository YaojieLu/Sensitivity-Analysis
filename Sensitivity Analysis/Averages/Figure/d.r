...
c <- 2.54
psi50 <- round(sapply(data1$d, Psi50fd), 2)
...
# average A
plotBy(data1$averA ~ psi50 | k, data=data1,
...
axis(2, xlim=c(0, 15), pos=-5, lwd=2, at=c(0, 5, 10, 15))
...
# E/MAP
plotBy(data1$EMAP ~ psi50 | k, data=data1,
       type='p', pch=19, legend=FALSE, legendwhere="topleft",
       xlim=c(-5, 0), ylim=c(0, 1),
       xlab=NA, ylab=NA,
       xaxt="n", yaxt="n",
       cex.lab=1.3, col=Cols)

axis(1, xlim=c(-5, 0), pos=0, lwd=2)
axis(2, xlim=c(0, 1), pos=-5, lwd=2)
mtext(expression(italic(psi[x50])~(MPa)), side=1, line=2.4, cex=1.3)
