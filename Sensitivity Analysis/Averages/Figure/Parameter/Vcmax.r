...
# average A
plotBy(data$averA ~ data$Vcmax, data=subset(data, k=="0.1" & MAP=="1900"),
...
plotBy(data$averB ~ data$Vcmax, data=subset(data, k=="0.1" & MAP=="1900"),
       type='l', legend=FALSE, legendwhere="topleft",
       xlim=c(0, 100), ylim=c(0, 20),
       xlab=NA, ylab=NA,
       xaxt="n", yaxt="n",
...
mtext(expression(italic(V[cmax])~(mu*mol~m^-2~s^-1)), side=1, line=2.5, cex=1.3)
