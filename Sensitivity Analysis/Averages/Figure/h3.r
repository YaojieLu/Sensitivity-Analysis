...
# average A
plotBy(data$averA ~ data$MAP | h3, data=subset(data, k=="0.025" & h3 %in% c("5", "10", "20")),
...
plotBy(data$averA ~ data$MAP | h3, data=subset(data, k=="0.1" & h3 %in% c("5", "10", "20")),
       type='l', legend=FALSE, legendwhere="topleft",
       xlim=c(0, 3000), ylim=c(0, 25),
       xlab=NA, ylab=NA,
       xaxt="n", yaxt="n",
...
legend("topleft", title=expression(italic(h[3])), c("5", "10", "20"), col=Cols, pch=19)

# E/MAP
plotBy(data$EMAP ~ data$MAP | h3, data=subset(data, k=="0.025" & h3 %in% c("5", "10", "20")),
       type='l', legend=FALSE, legendwhere="topleft",
...
plotBy(data$EMAP ~ data$MAP | h3, data=subset(data, k=="0.1" & h3 %in% c("5", "10", "20")),
       type='l', legend=FALSE, legendwhere="topleft",
       xlim=c(0, 3000), ylim=c(0, 1),
       xlab=NA, ylab=NA,
       xaxt="n", yaxt="n",
